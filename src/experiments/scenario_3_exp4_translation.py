"""
Scenario 3, Experiment 4: sensitivity of car/node object-geolocation agreement
to a synthetic translation error injected into the node LiDAR->GPS transform.

Merged from the three near-identical originals car_node_scenario_3_exp_4_{x,y,z}_translation.py,
which differed only in which axis the translation offset was applied to. Run with
`--axis x|y|z`.
"""
import argparse
import pickle
...
def get_axis_translation(axis, value):
    if axis == 'x':
...
def geo_localize_node_objects(geo_transformer, df_node_annotations, compass_heading, axis, value, delay):
    ...
    for index, row in df_node_annotations.iterrows():
        ...
        if shifted_time == 0:
            ...
            translation = get_axis_translation(axis, value)
            lidar_translated_point = lidar_point + translation

            gps_point = geo_transformer.node_geolocate_object(GPS_base_curr, compass_heading, lidar_translated_point)
            ...
            df_node_object_geolocation = pd.concat([df_node_object_geolocation, pd.DataFrame({
                ...
                'X': [lidar_translated_point[0]],
                'Y': [lidar_translated_point[1]],
                'Z': [lidar_translated_point[2]]
            })], ignore_index=True)
...
def plot_diff_df_interactive_with_averages(diff_df, axis):
    distance_ranges = [(8, 9), (9, 10), (10, 11), (11, 12), (12, 13), (13, 14), (14, 15),
    ...
    for idx, row in diff_df.iterrows():
        ...
        fig.add_trace(go.Scatter(
            x=delays, y=diff_values, mode='lines+markers',
    ...
    for range_label, values in range_averages.items():
        ...
        fig.add_trace(go.Scatter(x=delays, y=avg_values, mode='lines+markers', name=f'Average ({range_label})', visible=True))

    fig.update_layout(
        title='Difference in Car-to-Node Object Distance (Averaged and Individual Rows)',
        xaxis_title=f'{axis.upper()}_Translation (m)',
        ...
        xaxis=dict(tickmode='array', tickvals=delays, showgrid=True, tickangle=90),
...
def plot_diff_df_with_averages(diff_df, axis):
    ...
    for range_label in range_labels:
        if range_label in range_averages:
            plt.plot(delays, range_averages[range_label], label=range_label, marker='o')

    plt.title(f'Impact of {axis.upper()} Axis Translation Error on Spatial Uncertainty', fontsize=16)
    plt.xlabel(f'{axis.upper()}_Translation (m)', fontsize=16)
    plt.ylabel('Difference in Car-to-Node Object Distance (m)', fontsize=16)
    plt.xticks(delays, rotation=90, fontsize=14)
...
def main(axis):
    ...
    trans_list = np.round(np.arange(-5, 5, 0.1), 2)
    ...
    for trans in trans_list:
        ...
        df_node_object_geolocation = geo_localize_node_objects(geo_transformer, df_node_annotations, compass_heading, axis, trans, delay_node)
        ...
        df_merged_node_car_object_dict[trans] = df_merged_node_car_object
        eucl_dist_dict[trans] = calculate_euclidean_distances(wgs84, df_merged_node_car_object)

    relative_error_diff_df = calculate_difference_on_relative_error(eucl_dist_dict)

    with open(f'pickles/scenario_3_exp_4_{axis}_t.pkl', 'wb') as f:
        pickle.dump({
...
if __name__ == '__main__':
    ...
    parser.add_argument('--axis', choices=['x', 'y', 'z'], required=True, help='Translation axis to sweep')
